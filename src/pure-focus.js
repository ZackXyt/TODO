...
function _pfMkParts() {
  ...
  for(let i=0;i<60;i++){
    ...
    _pfParts.push({orbital:true, a, av:(Math.random()<.5?1:-1)*(.00025+Math.random()*.00045),
      r, rb:r, rd:0, sz, col:cs[i%3], t:Math.random()*Math.PI*2, tw:Math.random()*Math.PI*2, twv:.018+Math.random()*.022});
  }
...
function _pfFrame(ts) {
  ...
  const R=_pfCircleR>10 ? _pfCircleR : Math.min(W,H)*.18;
  const circY=cy-H*.05;
  ctx.save(); ctx.translate(cx,circY); ctx.scale(1+bp, 1+bp);
  const gl=ctx.createRadialGradient(0,0,R*.6,0,0,R*1.28);
  ...
  const progFrac=tm.duration>0?Math.max(0,tm.time/tm.duration):1;
